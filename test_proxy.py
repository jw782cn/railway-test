...
def test_create_proxy(object_key, add_text=True):
    ...
    # 准备请求数据
    payload = {"object_key": object_key, "add_text": add_text}
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("\n文件信息:")
            print(
                f"原始文件: {result['data']['original']['bucket']}/{result['data']['original']['key']}"
            )
            print(
                f"代理文件: {result['data']['proxy']['bucket']}/{result['data']['proxy']['key']}"
            )
            return result
        else:
